    ...
    <div class="view-container">
      ...
      >
        ...
        <el-table-column width="80" align="center" prop="hidden" label="是否隐藏">
          <template slot-scope="scope">
            <el-switch :value="scope.row.hidden == '1'" @change="chgHidden(scope.row.id, scope.row.hidden)" />
          </template>
        </el-table-column>

        <el-table-column width="80" align="center" prop="noCache" label="是否缓存">
          <template slot-scope="scope">
            <el-switch :value="scope.row.noCache == '0'" @change="chgCache(scope.row.id, scope.row.noCache)" />
          </template>
        </el-table-column>

        <el-table-column width="80" align="center" prop="affix" label="是否固定在标签页">
          <template slot-scope="scope">
            <el-switch :value="scope.row.affix == '1'" @change="chgAffix(scope.row.id, scope.row.affix)" />
          </template>
        </el-table-column>
        ...
        <el-table-column width="80" align="center" prop="breadcrumb" label="是否显示在面包屑">
          <template slot-scope="scope">
            ...
            <el-switch :value="scope.row.pid != 0 && scope.row.breadcrumb == '1'" @change="chgBreadcrumb(scope.row.id, scope.row.breadcrumb)" />
          </template>
        </el-table-column>
